...
def show_bqm_graph(bqm, show=True, ax=None):
    G = make_bqm_graph(bqm)
    ...
    nx.draw(G, pos=node_positions, width=biases, edge_color=colors, ax=ax)
    ...
    nx.draw_networkx_labels(
        G, pos=node_positions_offset, labels=latexed_node_labels, ax=ax
    )
    nx.draw_networkx_labels(G, pos=node_positions, labels=node_biases, ax=ax)
    nx.draw_networkx_edge_labels(
        G, node_positions, edge_labels=edge_labels, label_pos=0.4, ax=ax
    )
    if show:  # does not work if ax is provided
        plt.show()
