...
def calculate_S(nodes, basis, x_l=0, x_r=1, p=1, q=0, f=0):
    ...
    for i in range(len(nodes) - 1):  # i=0  here represents i=1 in the article
        ...
        s[i - 1, :] = [
            integrate.quad(lambda x: 1 / 2 * p(x) * phi_i_minus_1_prime(x) ** 2 + 1 / 2 * q(x) * phi_i_minus_1(x) ** 2,
                           nodes[i - 1], nodes[i])[0],
            integrate.quad(lambda x: 1 / 2 * p(x) * phi_i_prime(x) ** 2 + 1 / 2 * q(x) * phi_i(x) ** 2,
                           nodes[i - 1], nodes[i])[0],
            integrate.quad(
                lambda x: p(x) * phi_i_minus_1_prime(x) * phi_i_prime(x) + q(x) * phi_i_minus_1(x) * phi_i(x),
                nodes[i - 1], nodes[i])[0],
            integrate.quad(lambda x: - 1 * f(x) * phi_i_minus_1(x),
                           nodes[i - 1], nodes[i])[0],
            integrate.quad(lambda x: - 1 * f(x) * phi_i(x),
                           nodes[i - 1], nodes[i])[0]
            ]
    return s
